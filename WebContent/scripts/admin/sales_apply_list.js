$(function(){
	var url = HOST + 'api/admin/getSalesApplys?companyId=&status=&keyword=&orderby=';
	var method = 'POST';
	var pagination = new AtlantPagination(url, function(response) {
		...
		for (var i = 0; i < result.length; i++) {
			...
			html += '	<td>' + getBasicDataLabel(BasicData_SalesApplyStatus,item.status) +'</td>';
		...
		$('.btn-edit').click(function(){
			...
			location.href = HOST + 'admin/sales_apply_edit?salesApplyId=' + sid;
		});
